...
const EditFood = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://127.0.0.1:8000/api/edit_food/${id}/`)
      .then((response) => response.json())
      .then((data) => {
        setFormData(data);
      })
      .catch((error) => console.error('Error editing categories:', error));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append('is_available', formData.is_available ? 'true' : 'false');
      if (formData.image) {
        fd.append('image', formData.image);
      }

      const response = await fetch(
        `http://127.0.0.1:8000/api/edit_food/${id}/`,
        {
          method: 'PUT',
          body: fd,
        },
